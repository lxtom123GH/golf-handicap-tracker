...
|---|---|---|---|---|
| BL-DD-01 | 🔴 High | needs-design | No rate-limit on any AI callable → billing abuse (F8) | Per-user quota / App Check across all 5 Gemini callables; the practice-plan cache guard alone is insufficient (`functions/index.js:28,50,75,131`). **Rec (2026-06-26): App Check first (chunk a), then per-user daily Firestore quota (chunk b)** — App Check is the highest-leverage, lowest-code blocker for the real threat (scripted abuse of the billed key from outside the app) and ships in one chunk; the daily cap then bounds worst-case spend from a logged-in abuser. *Caveat:* App Check needs client wiring + a reCAPTCHA/Play-Integrity provider and debug tokens for emulator dev. |
