...
|---|---|---|---|---|
| BL-DD-01 | 🔴 High | needs-design | No rate-limit on any AI callable → billing abuse (F8) | Per-user quota / App Check across all 5 Gemini callables; the practice-plan cache guard alone is insufficient (`functions/index.js:28,50,75,131`) |
| BL-DD-02 | 🔴 High | needs-design | `xlsx@0.18.5` (proto-pollution/ReDoS, no fix) bundled to browser (F10) | **Rec (2026-06-26): drop `xlsx` → CSV-only import** (the CSV path already exists in `admin.js`); if `.xlsx` is a hard requirement, parse it **server-side** in a Cloud Function. *Why:* only removing it takes the vulnerable code out of the browser entirely (vs vetting a new dep or sandboxing), and it doubles as a bundle win (F26). (`admin.js:9`) |
| BL-DD-03 | 🟡 Med | clean | `askAiCoach` passes raw client prompt — no systemInstruction/length cap (F7) | Add coach-scoped `config.systemInstruction` + prompt-length cap (`functions/index.js:28-39`) |
| BL-DD-04 | 🟡 Med | needs-design | `comp_rounds` read = any-authenticated → all comp rounds globally readable (F17) | Constrain read to owner/invitee/participant; drop client-only compId narrowing (`firestore.rules:122-123`) |
| BL-DD-05 | 🟡 Med | needs-design | `users` read exposes email/isAdmin/isApproved to any authed user (F21) | Field-minimize / split public-profile reads (readers: social/admin/competitions) (`firestore.rules:32`) |
| BL-DD-06 | 🟡 Med | needs-design | `generateAudioBriefing` no object-ownership check + no size cap (F23) | Path-prefix ownership (`audio_diaries/{uid}/`) + size cap before base64 (`functions/index.js:143-164`); overlaps **BL-3.17** |
| BL-DD-07 | 🟡 Med | clean | `whs_rounds` create rule lacks `\|\| isAdmin()` → admin bulk-import denied (F19) | Add `\|\| isAdmin()` to `firestore.rules:81` (match line 82) |
...
| BL-DD-09 | 🟡 Med | clean | `onRoundCreated` unbounded `collectionGroup('following')` scan per round (F9) | Constrain the query to the author's followers (`functions/index.js:417`) |
| BL-DD-10 | 🟡 Med | clean | dead id `log-comp-dynamic` not null-safe → `innerHTML=''` on null (F30) | Repoint to `comp-dynamic-inputs` or null-guard (`competitions.js:187-188`) |
