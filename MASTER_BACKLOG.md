...
| BL-DD-03 | 🟡 Med | clean → **Tranche 1** ⏳ | `askAiCoach` passes raw client prompt — no systemInstruction/length cap (F7) | Add coach-scoped `config.systemInstruction` + prompt-length cap (`functions/index.js:28-39`). Brief: `docs/agent-briefs/bl-dd-tranche-1-clean-wins.md` |
...
| BL-DD-07 | 🟡 Med | clean → **Tranche 1** ⏳ | `whs_rounds` create rule lacks `\|\| isAdmin()` → admin bulk-import denied (F19) | Add `\|\| isAdmin()` to `firestore.rules:81` (match line 82). Brief: `docs/agent-briefs/bl-dd-tranche-1-clean-wins.md` |
